...
def get_score_details(game, team):
    if game["team_a"] == team['name']:
        game['place'] = 'home'
        try:
            game[f"nb_goals_{team['name']}"] = int(game['score'].split('-')[0])
            game['nb_goals_adv'] = int(game['score'].split('-')[1])
        except:pass
    else:
        game['place'] = 'away'
        try:
            game[f"nb_goals_{team['name']}"] = int(game['score'].split('-')[1])
            game['nb_goals_adv'] = int(game['score'].split('-')[0])
        except:pass
    try:
        if game[f"nb_goals_{team['name']}"] > game['nb_goals_adv']:
            game['result'] = 'WIN'
        elif game[f"nb_goals_{team['name']}"] == game['nb_goals_adv']:
            game['result'] = 'TIE'
        else:
            game['result'] = 'LOST'
    except:pass
    ...
    try:
        for col in columns:
            if row['team_a'] == team_name:
                row[
                    f"{col.lower().replace(' ', '_')}_{team_name.lower()}"
                ] = row[col]['team_a']

                row[f"{col.lower().replace(' ', '_')}_adv"] = row[col]['team_b']
            else:
                row[
                    f"{col.lower().replace(' ', '_')}_{team_name.lower()}"
                ] = row[col]['team_b']

                row[f"{col.lower().replace(' ', '_')}_adv"] = row[col]['team_a']
    except: pass
    return row
...
def convert_team_name(row, team_name):
    for col in ['goals', 'players_team', "subs_in"]:
        try:
            if row['team_a'] == team_name:
                row[f'{col}_{team_name.lower()}'] = row[f'{col}_a']
                row[f'{col}_adv'] = row[f'{col}_b']
            else:
                row[f'{col}_{team_name.lower()}'] = row[f'{col}_b']
                row[f'{col}_adv'] = row[f'{col}_a']
            del row[f'{col}_a'], row[f'{col}_b']
        except: pass
    return row    
                    ...
                    for e in (
                        list(
                            df[df.opponent == opponent][
                                f"players_team_{team_name.lower()}"
                            ]
                        )
                        + list(
                            df[df.opponent == opponent][
                                f"subs_in_{team_name.lower()}"
                            ]
                        )
                    )
                    if player in e
                ...
                goals = len(
                    [
                        ...
                        for e in np.concatenate(
                            [
                                elem
                                for elem in df[df.opponent == opponent][
                                    f"goals_{team_name.lower()}"
                                ]
                                if type(elem) == list
                            ]
                        )
...
def ratio_one_opponent(df, opponent, team_name, top_k=10):
    ...
    game_player = dict(
        Counter(
            np.concatenate(
                (
                    list(df_oppo[f"players_team_{team_name.lower()}"])
                    + list(df_oppo[f"subs_in_{team_name.lower()}"])
                )
            )
        ).items()
    )
    ...
    goal_counter = Counter(
        [
            ...
            for elem in np.concatenate(
                list(df_oppo[f"goals_{team_name.lower()}"])
            )
        ]
    )
...
def get_goals_team(bs, team):
    if bs.find('span', class_='bidi').text != ' 0 - 0':
        goals = [
            {
                'player': elem.find('a').text,
                'time': elem.find('span', class_='minute').text.replace(
                    "'", ''
                ),
                'assist': elem.find('span', class_='assist'),
            }
            for elem in bs.find('div', class_='block_match_goals').find_all(
                'td', class_=f'player-{team}'
            )
            if elem.text != '\n\n'
        ]

        for goal in goals:
            if goal['assist'] is None:
                del goal['assist']
            else:
...
def get_lineups(bs):
    ...
    lineups = {
        ...
        "players_team_b": players[11:22],
        'subs_in_a': [
            elem[: elem.index('for')]
            for elem in players[22:29]
            if 'for' in elem
        ],
    }
...
def get_stats(bs):
    ...
    stats_list = [
        elem.text[1:-1].split('\n')
        for elem in bs2.find('table').find('tr').find_all('tr')[1:]
    ][::2]

    return {
        elem[1]: {'team_a': int(elem[0]), 'team_b': int(elem[2])}
