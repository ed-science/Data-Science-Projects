...
def get_imdb_urls(movie_budget, nb_elements=None):
    ...
    for movie in tqdm(movie_budget[1000:1000+nb_elements]):
        ...
        imdb_search_link = f"http://www.imdb.com/find?ref_=nv_sr_fn&q={title_url}&s=tt"
    ...
    for category in awards_category:
        for awards_type in awards_kept:
            ...
            for k, award in enumerate(awards_cat):
                parsed_awards[f'{awards_type}_{category}_{k + 1}'] = award["award"]
...
def parse_actors(movie):
    ...
    top_k = 3
    parsed_actors = {
        'total_cast_fb_likes': sum(
            actor['actor_fb_likes'] for actor in movie['cast_info']
        )
    ...
    for k, actor in enumerate(sorted_actors[:top_k]):
        if k < len(sorted_actors):
            parsed_actors[f'actor_{k + 1}_name'] = actor['actor_name']
            parsed_actors[f'actor_{k + 1}_fb_likes'] = actor['actor_fb_likes']
        else:
            parsed_actors[f'actor_{k + 1}_name'] = None
            parsed_actors[f'actor_{k + 1}_fb_likes'] = None
...
def parse_production_company(movie):
    ...
    for k, company in enumerate(production_companies):
        if k < len(movie['production_co']):
            parsed_production_co[f'production_co_{k + 1}'] = company
        else:
            parsed_production_co[f'production_co_{k + 1}'] = None
...
if __name__ == '__main__':
    nb_elements = int(sys.argv[1]) if len(sys.argv) > 1 else None
